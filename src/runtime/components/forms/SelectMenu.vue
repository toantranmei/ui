        ...
        <button :id="inputId" :class="selectClass" :disabled="disabled" type="button" v-bind="attrs">
          ...
          <span v-if="(isTrailing && trailingIconName) || $slots.trailing" :class="trailingWrapperIconClass">
            <slot name="trailing" :disabled="disabled" :loading="loading">
              <MeiIcon :name="trailingIconName" :class="trailingIconClass" aria-hidden="true" />
            </slot>
          </span>
        </button>
      </slot>
              ...
              <li :class="[uiMenu.option.base, uiMenu.option.rounded, uiMenu.option.padding, uiMenu.option.size, uiMenu.option.color, active ? uiMenu.option.active : uiMenu.option.inactive, selected && uiMenu.option.selected, optionDisabled && uiMenu.option.disabled]">
                <div :class="uiMenu.option.container">
                  <slot name="option" :option="option" :active="active" :selected="selected">
                    <MeiIcon v-if="option.icon" :name="option.icon" :class="[uiMenu.option.icon.base, active ? uiMenu.option.icon.active : uiMenu.option.icon.inactive, option.iconClass]" aria-hidden="true" />
                    <MeiAvatar
                ...
                <span v-if="selected" :class="[uiMenu.option.selectedIcon.wrapper, uiMenu.option.selectedIcon.padding]">
                  <MeiIcon :name="selectedIcon" :class="uiMenu.option.selectedIcon.base" aria-hidden="true" />
                </span>
              </li>
            </component>
...
import { select, selectMenu } from '#mei-ui/ui.config'
import { useId } from '#imports'

const config = mergeConfig<typeof select>(appConfig.meiUI.strategy, appConfig.meiUI.select, select)

const configMenu = mergeConfig<typeof selectMenu>(appConfig.meiUI.strategy, appConfig.meiUI.selectMenu, selectMenu)
...
export default defineComponent({
  ...
  setup (props, { emit, slots }) {
    ...
    const leadingIconClass = computed(() => {
      return twJoin(
        ui.value.icon.base,
        color.value && appConfig.meiUI.colors.includes(color.value) && ui.value.icon.color.replaceAll('{color}', color.value),
