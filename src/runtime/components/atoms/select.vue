...
import { select } from '#mei-ui/ui-configs'
    ...
    >
      <template v-for="(option, index) in normalizedOptionsWithPlaceholder">
        <optgroup
          ...
          :key="`${(option as any)[valueAttribute]}-optgroup-${index}`"
          :value="(option as any)[valueAttribute]"
          :label="(option as any)[optionAttribute]"
        >
          <option
            v-for="(childOption, index2) in option.children"
            :key="`${(childOption as any)[valueAttribute]}-${index}-${index2}`"
            :value="(childOption as any)[valueAttribute]"
            :selected="(childOption as any)[valueAttribute] === normalizedValue"
            :disabled="childOption.disabled"
            v-text="(childOption as any)[optionAttribute]"
          />
        </optgroup>
        <option
          v-else
          :key="`${(option as any)[valueAttribute]}-${index}`"
          :value="(option as any)[valueAttribute]"
          :selected="(option as any)[valueAttribute] === normalizedValue"
          :disabled="option.disabled"
          v-text="(option as any)[optionAttribute]"
        />
      </template>
    </select>
