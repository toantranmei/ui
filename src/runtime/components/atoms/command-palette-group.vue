...
<template>
  <div :class="ui.group.wrapper">
    <h2
      v-if="label"
      :class="ui.group.label"
    >
      {{ label }}
    </h2>

    <div
      :class="ui.group.container"
      :aria-label="group[groupAttribute]"
    >
      <HComboboxOption
      ...
      >
        <div
          :class="[
            ui.group.command.base,
            active ? ui.group.command.active : ui.group.command.inactive,
        ...
        >
          <div :class="ui.group.command.container">
            ...
            >
              <MeiIcon
                ...
                :class="[
                  ...
                  active
                    ? ui.group.command.icon.active
                    : ui.group.command.icon.inactive,
                  command.iconClass,
                ]"
                aria-hidden="true"
              />
              <MeiAvatar
                ...
                v-bind="{
                  size: ui.group.command.avatar.size,
                  ...command.avatar,
                }"
              ...
              <span
                ...
                :class="ui.group.command.chip.base"
            ...
            <div
              :class="[
                ui.group.command.label,
                command.disabled && ui.group.command.disabled,
              ]"
            >
              <slot
                :name="`${group.key}-command`"
                :group="group"
                :command="command"
                :active="active"
                :selected="selected"
              >
                <span
                  v-if="command.prefix"
                  class="flex-shrink-0"
                  :class="command.prefixClass || ui.group.command.prefix"
                >{{ command.prefix }}</span>

                <span
                  class="truncate"
                  :class="{
                    'flex-none': command.suffix || command.matches?.length,
                  }"
                >{{ command[commandAttribute] }}</span>

                <!-- eslint-disable-next-line vue/no-v-html -->
                <span
                  v-if="command.matches?.length"
                  class="truncate"
          ...
          <MeiIcon
            v-if="selected"
            :name="selectedIcon"
            :class="ui.group.command.selectedIcon.base"
            aria-hidden="true"
          />
          <slot
            v-else-if="
              active && (group.active || $slots[`${group.key}-active`])
            "
          ...
          >
            <span
              v-if="command.shortcuts?.length"
              :class="ui.group.command.shortcuts"
            >
              <MeiKbd
                v-for="shortcut of command.shortcuts"
                :key="shortcut"
              >{{
                shortcut
              }}</MeiKbd>
            ...
            <span
              v-else-if="!command.disabled && group.inactive"
              :class="ui.group.inactive"
            >{{ group.inactive }}</span>
          </slot>
        </div>
      </HComboboxOption>
