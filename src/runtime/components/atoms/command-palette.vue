...
import type { Button, Command, Group, Strategy } from '../../types'
...
const config = mergeConfig<typeof commandPalette>(
  appConfig.meiUI.strategy,
  appConfig.meiUI.commandPalette,
  commandPalette,
)
  ...
  setup(props, { emit, expose }) {
    ...
    onMounted(() => {
      setTimeout(() => {
        ...
        const popoverProvidesSymbols
          = Object.getOwnPropertySymbols(popoverProvides)
        comboboxApi.value
          = popoverProvidesSymbols.length
          && popoverProvides[popoverProvidesSymbols[0]]
      }, 200)
    })
    ...
    const groups = computed(() => {
      ...
      const groupedCommands: Record<string, Command[]> = results.value.reduce(
        (acc, command) => {
      ...
      const groups: Group[] = Object.entries(groupedCommands)
        .map(([key, commands]) => {
      ...
      const searchGroups = props.groups
        .filter(
          group => !!group.search && searchResults.value[group.key]?.length,
        )
        .map((group) => {
    ...
    const iconClass = computed(() => {
      return twJoin(
        ...
        (props.loading || isLoading.value)
        && props.loadingIcon
        && ui.value.input.icon.loading,
      )
    })
    ...
    function activateFirstOption() {
      setTimeout(() => {
        ...
        comboboxInput.value?.$el.dispatchEvent(
    ...
    >
      ...
      <HComboboxInput
        ...
        :class="[
          ...
          ui.input.height,
          ui.input.padding,
          icon && ui.input.icon.padding,
          closeButton && ui.input.closeButton.padding,
        ]"
      ...
      <MeiButton
        ...
        :class="ui.input.closeButton.base"
        @click="onClear"
      />
    </div>
