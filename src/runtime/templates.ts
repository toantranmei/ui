...
export default function createTemplates(nuxt = useNuxt()) {
  const template = addTemplate({
    ...
    getContents: () =>
      `export default ${JSON.stringify(nuxt.options.appConfig.meiUI?.colors)};`,
    write: true,
  });
  const typesTemplate = addTemplate({
    filename: "mei-ui-colors.d.ts",
    getContents: () =>
      `declare module '#mei-ui-colors' { const defaultExport: ${JSON.stringify(nuxt.options.appConfig.meiUI?.colors)}; export default defaultExport; }`,
    write: true,
  });
