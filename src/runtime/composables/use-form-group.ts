...
export function useFormGroup(inputProps?: InputProps, config?: any) {
  const formBus = inject<UseEventBusReturn<FormEvent, string> | undefined>(
    'form-events',
    undefined,
  )
  const formGroup = inject<InjectedFormGroupValue | undefined>(
    'form-group',
    undefined,
  )
  ...
  return {
    ...
    size: computed(() => {
      const formGroupSize = config.size[formGroup?.size.value as string]
